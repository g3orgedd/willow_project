...
function loadXml(xmlText) {
  ...
  elStatusLeft.textContent =
    `Loaded: ${state.docMeta.designedFor} • Size: ${internalToMm(state.subImage.width).toFixed(2)}×${internalToMm(state.subImage.height).toFixed(2)}mm` +
...
function renderGrid() {
  ...
  const wMm = internalToMm(state.subImage.width);
  const hMm = internalToMm(state.subImage.height);
